...
router.post("".concat(enums_1.Path.Root), function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, email, password, firstName, lastName, user, error_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 2, , 3]);
                _a = req.body, email = _a.email, password = _a.password, firstName = _a.firstName, lastName = _a.lastName;
                return [4 /*yield*/, (0, repository_1.createUser)({ firstName: firstName, lastName: lastName, email: email, password: password })];
            case 1:
                user = _b.sent();
                return [2 /*return*/, user
                        ? res.status(200).send({ user: user })
                        : res.status(400).send({ message: enums_1.ErrorMessage.EmailIsUse })];
            case 2:
                error_1 = _b.sent();
